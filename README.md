...
+ 克隆本项目，直接运行相应`src/ui.py`脚本即可（**代码内有详细注释**）
...
CMCompiler 实现源码
|-- grammar 语法分析
|   |-- c_grammar.py 语法分析器实现（无界面接口）
|   |-- c_grammar_uiapi.py 语法分析器实现（有界面接口）
|   |-- test.c 测试输入
|   |-- ui.py 界面实现
|   `-- yacc_ui.py 带界面接口的yacc
|-- lexer 词法分析
|   |-- c_lexer.py 词法分析器实现（可独立运行）
|   |-- output.txt 输出文件
|   `-- test_input 测试输入文件
|-- ply PLY模块，从官网下载，不要轻易修改它
|   |-- lex.py
|   |-- yacc.py
|   `-- yacc_ui.py 修改后的yacc（带界面接口）
|-- semantic 语义分析模块（集成词法，语法，语义，界面）
|   |-- __pycache__
|   |-- sematic_uiapi.py
|   |-- test_input
|   |-- ui.py
|   `-- yacc_ui.py
`-- src 最终项目代码（集成词法，语法，语义，界面）
|   |-- sematic_uiapi.py 词法，语法，语义（类型系统，错误处理），汇编生成
|   |-- test_input 测试输入文件目录
|   |-- ui.py 界面实现（这里是启动入口）
|   `-- yacc_ui.py 带界面接口的yacc
└─DesignDocument	设计文档
